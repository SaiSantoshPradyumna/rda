...
public class ingleLauncher extends JFrame {
    ...
    static class HostGUI extends JFrame {
        ...
        public HostGUI(int p) {
            ...
            screenLabel = new JLabel(); screenLabel.setHorizontalAlignment(SwingConstants.CENTER);
            screenLabel.setOpaque(true); screenLabel.setBackground(Color.BLACK);
            ...
            screenLabel.addMouseListener(ma); screenLabel.addMouseMotionListener(ma);
        ...
        private void forward(MouseEvent e) { ClientHandler s = (ClientHandler)combo.getSelectedItem(); if(s != null) s.sendMouseEvent(e, screenLabel.getWidth(), screenLabel.getHeight()); }
        private void forwardKey(KeyEvent e){ ClientHandler s = (ClientHandler)combo.getSelectedItem(); if(s != null) s.sendKeyEvent(e); }
    }
    ...
    static class ClientHandler extends Thread {
        private Socket s; private HostGUI gui; private ObjectInputStream in; private ObjectOutputStream out;
        private ImageIcon last; private String name = "Unknown Client"; private ChatWindow chat;
                            ...
                            SwingUtilities.invokeLater(() -> {
                                ...
                                if(c == JOptionPane.YES_OPTION){
                                    ...
                                    if(fc.showSaveDialog(gui) == JFileChooser.APPROVE_OPTION){
                                        ...
                                        try(FileOutputStream fos = new FileOutputStream(sf)){ fos.write(m.fileData); } catch(IOException ex){}
                                    }
                                }
                            });
                        }
    ...
    static class RemoteDesktopClient {
        ...
        private ClientGUI clientGUI; private ConnectionEntry conn; private boolean running = false;
        private ClientLauncher launcher; private ClientChatWindow chatWindow; 
        private Robot robot;
                                    ...
                                    SwingUtilities.invokeLater(() -> {
                                        ...
                                        if(c == JOptionPane.YES_OPTION) {
                                            ...
                                            if(fc.showSaveDialog(clientGUI) == JFileChooser.APPROVE_OPTION) {
                                                File sf = fc.getSelectedFile();
                                                try(FileOutputStream f = new FileOutputStream(sf)) { f.write(m.fileData); } catch(IOException x){}
                                            }
                                        }
                                    });
                                }
        ...
        public void stopConnection() {
            ...
            SwingUtilities.invokeLater(() -> {
                if(clientGUI != null) { clientGUI.showConnectionStopped(); clientGUI.dispose(); }
    ...
    static class ClientGUI extends JFrame {
        ...
        public ClientGUI(){
            ...
            p.add(openChatButton); p.add(sendFileButton); p.add(stopConnButton);
            add(p, BorderLayout.SOUTH);
        }
    ...
    static class ClientChatWindow extends JFrame {
        ...
        public ClientChatWindow(RemoteDesktopClient c) {
            super("Chat");
            controller = c; setSize(600,400); setLocationRelativeTo(null);
            setLayout(new BorderLayout());
            setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
            area = new JTextArea(); area.setEditable(false);
            add(new JScrollPane(area), BorderLayout.CENTER);
    ...
    static class ClientLauncher extends JFrame {
        ...
        public ClientLauncher(){
            ...
            connectBtn.addActionListener(e -> {
                ...
                catch(Exception ex){ JOptionPane.showMessageDialog(this, "Invalid port"); return; }
                ConnectionEntry sel = null;
                for(int i = 0; i < model.getSize(); i++){
                    ConnectionEntry c = model.getElementAt(i);
                    if(c.getIp().equals(ip) && c.getPort() == p){ sel = c; break; }
                }
        ...
        private java.util.List<ConnectionEntry> getAll(){
            java.util.List<ConnectionEntry> l = new ArrayList<>();
